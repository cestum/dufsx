...
let createChunEditor;
let createImageUploadPlugin;

let imageUploadPlugin;
...
async function ready() {
  ...
  createChunEditor = Chun.createChunEditor;
  createImageUploadPlugin = Chun.createImageUploadPlugin;

  imageUploadPlugin = createImageUploadPlugin({
    imageUploadUrl: "",
...
async function loadFileContent(path, fileInfo) {
  try {
    ...
    if (DATA.allow_delete) {
      ...
      $deleteFile.onclick = () => deleteCurrentFileFolder(currentFilePath);
    }

    if (DATA.allow_upload) {
      const $moveFile = document.querySelector('.move-file');
      $moveFile.classList.remove('hidden');
      $moveFile.onclick = () => moveCurrentFileFolder(currentFilePath);
    }
...
function setupMarkdownEditor2(content) {
  $markdownEditorContainer.classList.remove('hidden');
  ...
  const editor = createChunEditor({
    ...
    .mount("markdown-editor-container")
...
function createEditorToolbar() {
  ...
  toolbar.innerHTML = `
    <button id="format-btn" title="Format Document (Ctrl+Shift+F)">Format</button>
    <span class="file-info text-sm text-gray-400" id="editor-info">Ready</span>
...
async function deleteCurrentFileFolder(thisFileFolder) {//currentFilePath
  if (!thisFileFolder) return;

  const name = baseName(thisFileFolder);
  if (!confirm(`Delete \`${name}\`?`)) return;

  try {
    await checkAuth();
    const url = `${DATA.relative_uri_prefix}${encodeURIComponent(thisFileFolder)}`;
...
async function moveCurrentFileFolder(__currentFilePath) {
  console.log("Trying to move ", __currentFilePath);

  if (!__currentFilePath) return;

  const newPath = prompt('Enter new path:', __currentFilePath);
  if (!newPath || newPath === __currentFilePath) return;

  try {
    await checkAuth();
    const url = `${DATA.relative_uri_prefix}${encodeURIComponent(__currentFilePath)}`;
...
function createTreeItem(item, path, isDirectory) {
  ...
  icon.innerHTML = getPathSvg(item.path_type);
...
async function showDirectoryContents(dirPath) {
  try {
            ...
            <tbody>
              ${data.paths.map(item => `
                <tr class="directory-item" data-path="${dirPath ? dirPath + '/' + item.name : item.name}" data-type="${item.path_type}">
                  <td class="item-name">
                    <div class="item-info">
                      <span class="item-icon">${getPathSvg(item.path_type)}</span>
                  ...
                  <td class="item-modified">${formatMtime(item.mtime)}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>` :
    ...
    if (DATA.allow_delete) {
      const $deleteFile = document.querySelector('.delete-file');
      $deleteFile.classList.remove('hidden');
      $deleteFile.onclick = () => deleteCurrentFileFolder(selectedDirectory);
    }

    if (DATA.allow_upload) {
      const $moveFile = document.querySelector('.move-file');
      $moveFile.classList.remove('hidden');
      $moveFile.onclick = () => moveCurrentFileFolder(selectedDirectory);
    }
